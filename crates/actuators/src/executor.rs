...
pub fn apply_transaction<S: KvStore>(
    ...
    match contract.r#type() {
        ...
        ContractType::UpdateAssetContract => {
            let c: protocol::UpdateAssetContract = unpack(contract)?;
            let actuator = crate::update_asset::UpdateAssetActuator::new(&c);
            actuator.validate(state)?;
            actuator.execute(state)
        }
        ContractType::UnfreezeAssetContract => {
            let c: protocol::UnfreezeAssetContract = unpack(contract)?;
            let actuator = crate::unfreeze_asset::UnfreezeAssetActuator::new(&c);
            actuator.validate(state)?;
            actuator.execute(state)
        }
        ...
        ContractType::MarketSellAssetContract => {
            let c: protocol::MarketSellAssetContract = unpack(contract)?;
            let actuator = crate::market::sell::MarketSellAssetActuator::new(&c);
            actuator.validate(state)?;
            actuator.execute(state)
        }
        ContractType::MarketCancelOrderContract => {
            let c: protocol::MarketCancelOrderContract = unpack(contract)?;
            let actuator = crate::market::cancel::MarketCancelOrderActuator::new(&c);
            actuator.validate(state)?;
            actuator.execute(state)
        }
