...
pub fn router_with_state<S: KvStore + 'static>(state: NodeState<S>) -> Router {
    Router::new()
        ...
        .route("/wallet/getassetissuelist", post(get_asset_issue_list::<S>))
        .route("/wallet/getassetissuebyid", post(get_asset_issue_by_id::<S>))
        .route("/wallet/getassetissuebyname", post(get_asset_issue_by_name::<S>))
        .route("/wallet/getassetissuelistbyname", post(get_asset_issue_list_by_name::<S>))
...
async fn get_asset_issue_by_name<S: KvStore>(State(state): State<AppState<S>>, Json(req): Json<Value>) -> Json<Value> { Json(http::assets::get_asset_issue_by_name(&state, &req)) }
async fn get_asset_issue_list_by_name<S: KvStore>(State(state): State<AppState<S>>, Json(req): Json<Value>) -> Json<Value> { Json(http::assets::get_asset_issue_list_by_name(&state, &req)) }
async fn get_paginated_asset_issue_list<S: KvStore>(State(state): State<AppState<S>>, Json(req): Json<Value>) -> Json<Value> { Json(http::assets::get_paginated_asset_issue_list(&state, &req)) }
...
async fn get_asset_issue_list<S: KvStore>(State(state): State<AppState<S>>) -> Json<Value> { Json(http::assets::get_asset_issue_list(&state)) }
